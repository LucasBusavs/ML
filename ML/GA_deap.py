...
def fitness_function(individual):
    ...
    model = KNeighborsClassifier(
        n_neighbors=n_neighbors_val, weights=weight_val, p=p_val)
...
def mutate(individual):
    ...
    print(
        f"Indivíduo mutado para n_neighbors = {n_neighbors_val}, weights = {weight_val}, p = {p_val}")
    return individual,
...
def crossover(ind1, ind2):
    print(
        f"Antes do crossover: {ind1[0]} - {ind2[0]} | {ind1[1]} - {ind2[1]} | {ind1[2]} - {ind2[2]}")
    ...
    print(
        f"Após crossover: {ind1[0]} - {ind2[0]} | {ind1[1]} - {ind2[1]} | {ind1[2]} - {ind2[2]}")
    return ind1, ind2
...
creator.create("FitnessMax", base.Fitness,
               weights=(1.0,))  # Maximizar a fitness
...
# Usando a função de crossover personalizada
...
algorithms.eaSimple(population, toolbox, cxpb=0.7, mutpb=0.2,
                    ngen=40, stats=None, halloffame=None, verbose=True)

best_individual = tools.selBest(population, 1)[0]
print(
    f"Melhor valor de K encontrado: {best_individual[0]}, melhor valor de weights: {best_individual[1]}, melhor valor de p: {best_individual[2]}")
